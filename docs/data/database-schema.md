...
| team_id       | foreign key teams.id nullable | Team hiện tại của user. Mỗi user chỉ thuộc 0 hoặc 1 team |
...
- `users_team_id_index` trên `team_id`

Quan hệ team:

- Một user có thể thuộc về một team qua `team_id`.
- Một user có thể là Lead của nhiều team về mặt schema, nhưng business rule hiện tại nên giới hạn bằng policy/request theo nhu cầu vận hành. Nếu muốn cứng hơn ở DB, có thể thêm unique index cho `teams.lead_id`.

## Bảng teams

Lưu cấu trúc team nội bộ để Admin hoặc Lead quản lý thành viên.

| Cột         | Kiểu dữ liệu gợi ý            | Mô tả                                                        |
| ----------- | ----------------------------- | ------------------------------------------------------------ |
| id          | UUID / bigint                 | Khóa chính                                                   |
| name        | varchar                       | Tên team                                                     |
| description | text nullable                 | Mô tả team                                                   |
| lead_id     | foreign key users.id nullable | User đang phụ trách team. Optional, team có thể chưa có Lead |
| created_by  | foreign key users.id nullable | Admin tạo team                                               |
| created_at  | timestamp                     | Thời điểm tạo                                                |
| updated_at  | timestamp                     | Thời điểm cập nhật                                           |

Index gợi ý:

- `teams_lead_id_index` trên `lead_id`
- `teams_created_by_index` trên `created_by`

Quan hệ:

- Một team có 0 hoặc 1 Lead qua `lead_id`.
- Một team có nhiều member qua `users.team_id`.
- Lead của team bắt buộc phải là một user đang thuộc chính team đó khi được gán.
- Nếu Lead bị xóa, bị chuyển sang team khác, hoặc bị gỡ khỏi team hiện tại, `teams.lead_id` phải trở về `null`; không tự động chọn member khác thay thế.

Lý do thiết kế:

- Đặt `lead_id` trên `teams` vì mỗi team chỉ có 0 hoặc 1 Lead, dễ query `team->lead` và dễ gỡ Lead bằng cách set null.
- Đặt `team_id` trên `users` vì mỗi member chỉ thuộc 0 hoặc 1 team tại một thời điểm. Không dùng pivot vì pivot phù hợp cho quan hệ nhiều team/một user, trái với rule hiện tại.
...
- Một lead có thể được giao cho một sales hiện tại qua `assigned_to`; lịch sử giao lead lưu trong `lead_activities`.
...
- Chỉ Admin được tạo/xóa team và gán/gỡ Lead của team.
- Lead của team chỉ được thêm, xóa và mời member trong team mình phụ trách; Lead không được tự gán hoặc đổi Lead khác.
- Member thường không có quyền quản lý team.
- Khi member được thêm vào team khác, member tự rời team cũ bằng cách cập nhật `users.team_id`.
